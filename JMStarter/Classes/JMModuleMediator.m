...
@implementation JMModuleMediator

+ (instancetype)sharedInstance
{
    static JMModuleMediator* mediator = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        mediator = [[JMModuleMediator alloc] init];
    });
    return mediator;
}
...
- (id)implForProtocol:(Protocol*)protocol
{
    ...
    NSAssert([imp conformsToProtocol:@protocol(JMModuleBaseProtocol)], @"❌ImpL 实现的Protocol必须从QBModuleBaseProtocol继承!");
...
{
    return [[JMModuleMediator sharedInstance] implForProtocol:protocol];
}
